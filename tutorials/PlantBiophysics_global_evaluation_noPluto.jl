...
if !fitted
    ...
    df.VcMaxRef .= df.JMaxRef .= df.RdRef .= df.TPURef .= df.g0 .= df.g1 .= 0.
    df.VcMaxRefPE .= df.JMaxRefPE .= df.RdRefPE .= df.TPURefPE .= df.g0PE .= df.g1PE .= 0.
    for i in unique(df.Curve)
        ...
        if !ismissing(coefs[4])
            ...
            df.TPURefPE[df.Curve.==i] .= 1000000.
        end
...
index = ifelse.(df.PPFD .> 1400, index, 9999.)
filter!(x->x!=9999.,index)
indexPE = 1:length(df.PPFD)
indexPE = ifelse.((df.PPFD .> 1400).&(df.PEfailed .==0.), indexPE, 9999.)
...
# Leaf temperature
indexPE = 1:length(df.PPFD)
indexPE = ifelse.((df.PEfailed .==0.), indexPE, 9999.)
filter!(x->x!=9999.,indexPE)
